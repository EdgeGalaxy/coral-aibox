...
from coral import CoralNode, BaseParamsModel, FirstPayload, RawPayload, PTManager, NodeType
...
class AIboxCamera(CoralNode):
    ...
    def init(self, index: int, context: dict):
        ...
        if not self.process.enable_parallel:
            raise ValueError('当前摄像头只支持并行模式, 设置 process -> enable_parallel 为 true')

        if self.process.count != len(cameras):
            raise ValueError(f'摄像头数量与启动线程数不匹配, 设置 process -> count 为 {len(cameras)}')
    ...
    def sender(self, payload: RawPayload, context: Dict) -> FirstPayload:
        ...
        return FirstPayload(source_id=context['name'], raw=frame, raw_params=context['params'])
